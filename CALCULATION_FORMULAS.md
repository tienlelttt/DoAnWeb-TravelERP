...
Tài liệu này rà soát và tổng hợp lại tất cả các công thức tính toán/nghiệp vụ quan trọng nhất trong hệ thống Digital Travel ERP. Nó thể hiện sự đồng nhất giữa **Frontend** và **Backend** dựa trên nguyên tắc Backend là nguồn quyết định cuối cùng (Single Source of Truth).

---

## 1. Công thức tính Giá Vé (Pricing Formula)
**Mục đích**: Tính tổng tiền khi khách hàng đặt tour dựa trên độ tuổi, dịch vụ thêm và khuyến mÃi.
...
**Mục đích**: Tính số tiền được trừ khi áp dụng mãgiảm giá.
...
**Mục đích**: Tính phần trăm phí phạt và số tiền hoàn trả khi khách yêu cầu hủy tour (Tỷ lệ bậc thang theo số ngày còn lại đến lúc khởi hành).
...
## 4. Công thức Xếp Hạng & Điểm Xanh (Loyalty Points Formula)
...
*   **Backend**: `VanHanhService.php` (tự động tính khi tour kết thúc).

### 4.2. Trừ điểm và Hạ hạng (Inactivity Penalty)
*   **Công thức**: Nếu khách hàng không có hoạt động trong 6 tháng, bị hạ 1 bậc. Đồng thời điểm sẽ bị gọt về mức "trần" của bậc mới.
...
*   **Công thức**: Dựa vào `tinhDiemCanDoi` ở `VoucherService`. 
    ...
    *   Voucher %: `Mức_giảm_tối_đa * Phần_trăm * 2 / 100` (hoặc nhân 50 nếu không có trần).
...
## 5. Công thức Xếp Hạng Đánh Giá (Rating Average Formula)
...
*   **Sự đồng nhất**: **âœ… Đồng nhất**. Hệ thống dùng Moving Average để giảm thiểu tài nguyên CPU phải sum lại từ đầu. Frontend chỉ render dựa trên kết quả.

## 6. Công thức Tính Lợi Nhuận Kế Toán (Profit Formula)
**Mục đích**: Tính lÃi/lỗ của một tour sau khi đãvận hành xong.
...
### Kết luận
Toàn bộ hệ thống hiện đang sử dụng chung bộ công thức **rất chặt chẽ và đồng nhất**, không có sự xung đột logic nào làm lệch dữ liệu Database. Frontend thực hiện tốt vai trò hiển thị và trải nghiệm người dùng, nhường toàn quyền quyết định logic nghiệp vụ cốt lõi cho Backend xử lý.
